...
vector<pthread_mutex_t> mutexLocks;
...
int main (void)
{
    ...
    */
    ...
    /* Storing the starting time in milliseconds*/
    ...
    outputFile << "Fine-Grained Lock" << endl;
...
void colorPartition ( int index )
{
    ...
    for ( int i = 0 ; i < externalVertices.size() ; ++i ){
        ...
        /* Obtaining locks of neighbours and vertex itself */ 
        for ( int j = 0 ; j < n ; ++j ){
            if ( adjMatrix[externalVertices[i]][j] || externalVertices[i] == j ){
        ...
        /* Releasing locks of neighbours and vertex itself */
        for ( int j = 0 ; j < n ; ++j ){
            if ( adjMatrix[externalVertices[i]][j] || externalVertices[i] == j ){
